...
namespace ConsoleAppClient
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("Start to unlock " + DateTime.Now.ToString());
            WinStationClient client = new WinStationClient("nettcp", "net.tcp://localhost:9999/");
            client.UnLockWinStation("lcm", "880330", "", 1);
            // type = 0, will use default, but it's slow, because it needs to decrypt credentials
            client.Close();
            Console.WriteLine("End to unlock " + DateTime.Now.ToString());
...
//                        if (si.pWinStationName == "Console" && si.State == WTS_CONNECTSTATE_CLASS.WTSActive)
