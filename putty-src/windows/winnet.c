...
static Socket *sk_net_accept(accept_ctx_t ctx, Plug *plug)
{
	...
	assert(plug->hwnd);
    errstr = do_select(plug->hwnd, ret->s, true);
    if (errstr) {
	ret->error = errstr;
	return &ret->sock;
    }
...
static DWORD try_connect(NetSocket *sock)
{
	...
	do_select(sock->plug->hwnd, sock->s, false);
    ...
    /* Set up a select mechanism. This could be an AsyncSelect on a
     * window, or an EventSelect on an event object. */
    errstr = do_select(sock->plug->hwnd, s, true);
    if (errstr) {
	sock->error = errstr;
	err = 1;
	goto ret;
                       ...
                       bool local_host_only, int orig_address_family)
{
    ...
    /* Set up a select mechanism. This could be an AsyncSelect on a
     * window, or an EventSelect on an event object. */
    errstr = do_select(plug->hwnd, s, true);
    if (errstr) {
	p_closesocket(s);
	ret->error = errstr;
	return &ret->sock;
...
static void sk_net_close(Socket *sock)
{
    ...
    del234(sktree, s);
    do_select(s->plug->hwnd, s->s, false);
    p_closesocket(s->s);
    if (s->addr)
	sk_addr_free(s->addr);
...
void select_result(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    int ret;
    DWORD err;
...
static void sk_net_set_frozen(Socket *sock, bool is_frozen)
{
    ...
    s->frozen = is_frozen;
    if (!is_frozen) {
	do_select(s->plug->hwnd, s->s, true);
