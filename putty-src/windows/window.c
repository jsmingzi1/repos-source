...
#include "winsecur.h"
#include "tree234.h"
#include "windows/VS2012/putty/util.h"
...
/*GLOBAL*/ //HWND logbox;
///*GLOBAL*/ Terminal* term1;
///*GLOBAL*/ LogContext* logctx1;
///*GLOBAL*/ Conf* conf1;
///*GLOBAL*/ HWND hwnd1;
...
#define GET_WINDOW_DATA(hwindow) PWindowData pData = get_mapchain_value(window_nodes, hwindow);

static Mouse_Button translate_button(Mouse_Button button);
static LRESULT CALLBACK WndProc(HWND, UINT, WPARAM, LPARAM);
static int TranslateKey(HWND, UINT message, WPARAM wParam, LPARAM lParam,
			unsigned char *output);
static void conftopalette(void);
static void systopalette(void);
static void init_palette(HWND);
...
static bool is_full_screen(HWND);
static void make_full_screen(HWND hwnd);
static void clear_full_screen(HWND);
static void flip_full_screen(HWND);
static void process_clipdata(HWND hwnd, HGLOBAL clipdata, bool unicode);
static void setup_clipboards(Terminal *, Conf *);

/* Window layout information */
static void reset_window(HWND hwnd, int);
...
static void flash_window(HWND hwnd, int mode);
static void sys_cursor_update(HWND hwnd);
static bool get_fullscreen_rect(HWND, RECT * ss);
...
static UINT wm_mousewheel = WM_MOUSEWHEEL; //don't need change
...
static bool is_utf8(HWND hwnd)
{
	GET_WINDOW_DATA(hwnd);
    return pData->ucsdata.line_codepage == CP_UTF8;
}

static bool wintw_is_utf8(TermWin *tw)
{
    return is_utf8(tw->hWnd);
}

static bool win_seat_is_utf8(Seat *seat)
{
    return is_utf8(seat->hWnd);
}

char *win_seat_get_ttymode(Seat *seat, const char *mode)
{
	GET_WINDOW_DATA(seat->hWnd);
    return term_get_ttymode(pData->term, mode);
}

bool win_seat_get_window_pixel_size(Seat *seat, int *x, int *y)
{
	assert(seat->termwin);
    win_get_pixels(seat->termwin, x, y);
    return true;
}
...
{
	GET_WINDOW_DATA(seat->hWnd);
    return stripctrl_new_term(bs_out, false, 0, pData->term);
}
...
static void start_backend(TermWin* wintw)
{
    ...
    pData->window_name = pData->icon_name = NULL;
     ...
     */
    term_provide_backend(pData->term, pData->backend);
...
static void close_session(TermWin* wintw)
{
    char *newtitle;
    int i;
	assert(wintw);
    ...
    if (pData->backend) {
		...
		pData->backend = NULL;
        term_provide_backend(pData->term, NULL);
	seat_update_specials_menu(pData->win_seat);
    }
		...
		if (*p == '@') {
			...
			for (i = 0; i < argc; i++) {
				...
				ret = cmdline_process_param(p, i + 1 < argc ? argv[i + 1] : NULL,
					1, conf);
				if (ret == -2) {
					cmdline_error(NULL, "option \"%s\" requires an argument", p);
				}
				else if (ret == 2) {
					...
					//pgp_fingerprints(hwnd1);
					exit(1);
				}
				else if (*p != '-') {
					cmdline_error(NULL, "unexpected argument \"%s\"", p);
				}
				else {
					cmdline_error(NULL, "unknown option \"%s\"", p);
				}
			}
		}
	...
	while (1) {
		...
		if (toplevel_callback_pending() ||
			...
			/* The messages seem unreliable; especially if we're being tricky */
				term_set_focus(pData->term, GetForegroundWindow() == hwnd);
		}
...
finished:
	cleanup_exit(msg.hwnd, msg.wParam);	       /* this doesn't return... */
...
int NewWindow(int show)
{
	...
	int cursor_type;
	int vtmode;
    conf_cache_data(&cursor_type, &vtmode);
	...
	hwnd = CreateWindowExW(exwinmode, uappname, uappname,
        ...
        //sfree(uappname);
	/*hwnd1 = CreateWindowExW(exwinmode, uappname, uappname,
			winmode, CW_USEDEFAULT, CW_USEDEFAULT,
			guess_width, guess_height,
			NULL, NULL, inst, NULL);
	DWORD dwError = 0;
	if (hwnd1==NULL) {
		...
		MessageBoxA(NULL, "hwnd1 failed", "error", 0);
	}*/
	sfree(uappname);

	//win_seat->hWnd = hwnd;
	//win_seat->termwin = NULL;
    }
	...
	pData->font_height = font_height;
	pData->font_width = font_width;
	pData->was_zoomed = false;
	pData->cursor_type = cursor_type;
	pData->vtmode = vtmode;
	memcpy(&(pData->ucsdata), &ucsdata, sizeof(ucsdata));

	pData->win_seat = calloc(1, sizeof(Seat));
	pData->win_seat->vt = &win_seat_vt;
	//memcpy(&(pData->win_seat), &win_seat_vt, sizeof(win_seat_vt));
	pData->win_seat->hWnd = hwnd;
	pData->win_seat->termwin = NULL;
    init_fonts(hwnd, 0,0);
     ...
     */
	pData->win_seat->termwin = snew(struct TermWin);
	pData->win_seat->termwin->vt = &windows_termwin_vt;
	pData->win_seat->termwin->hWnd = hwnd;
	pData->win_seat->termwin->hdc = NULL;
    pData->term = term_init(conf, &(pData->ucsdata), pData->win_seat->termwin);
	pData->term->win = pData->win_seat->termwin;//init it
	pData->term->win->hWnd = hwnd; // init it
	//
    setup_clipboards(pData->term, conf);
    logctx = log_init(default_logpolicy, conf);
    term_provide_logctx(pData->term, logctx);
    term_size(pData->term, conf_get_int(conf, CONF_height),
	...
	pData->extra_width = wr.right - wr.left - cr.right + cr.left + offset_width*2;
	pData->extra_height = wr.bottom - wr.top - cr.bottom + cr.top +offset_height*2;
    }
     ...
     * to be.
     */
    guess_width = pData->extra_width + pData->font_width * pData->term->cols;
    guess_height = pData->extra_height + pData->font_height * pData->term->rows;
    ...
    {
	char *bits;
	int size = (pData->font_width + 15) / 16 * 2 * pData->font_height;
	bits = snewn(size, char);
	memset(bits, 0, size);
	pData->caretbm = CreateBitmap(pData->font_width, pData->font_height, 1, 1, bits);
	sfree(bits);
    }
    CreateCaret(hwnd, pData->caretbm, pData->font_width, pData->font_height);
	...
	si.nMin = 0;
	si.nMax = pData->term->rows - 1;
	si.nPage = pData->term->rows;
	si.nPos = 0;
     ...
     */
    pData->lastact = MA_NOTHING;
    pData->lastbtn = MBT_NOTHING;
    pData->dbltime = GetDoubleClickTime();
	...
	assert(pData->term->win);
    start_backend(pData->term->win);
 ...
 * Clean up and exit.
 */
void cleanup_exit(HWND hwnd, int code)
{
    /*
     * Clean up.
     */
 ...
 */
char *do_select(HWND hwnd, SOCKET skt, bool startup)
{
	assert(hwnd);
    int msg, events;
    if (startup) {
...
static void win_seat_update_specials_menu(Seat *seat)
{
    ...
    if (pData->backend)
        specials = backend_get_specials(pData->backend);
    else
	specials = NULL;
...
static void update_mouse_pointer(HWND hwnd)
{
    LPTSTR curstype = NULL;
    bool force_visible = false;
    static bool forced_visible = false;
	PWindowData pData = get_mapchain_value(window_nodes, hwnd);
    switch (pData->busy_status) {
      case BUSY_NOT:
	if (send_raw_mouse)
	    curstype = IDC_ARROW;
	else
...
{
	PWindowData pData = get_mapchain_value(window_nodes, seat->hWnd);
	pData->busy_status = status;
    //busy_status = status;
	assert(seat->hWnd);
    update_mouse_pointer(seat->hWnd);
}
...
static void win_seat_connection_fatal(Seat *seat, const char *msg)
{
    ...
    MessageBox(seat->hWnd, msg, title, MB_ICONERROR | MB_OK);
    sfree(title);
 ...
 */
void cmdline_error(HWND hwnd, const char *fmt, ...)
{
    va_list ap;
    char *message, *title;
...
static void wm_netevent_callback(void *vctx)
{
    ...
    select_result(params->hwnd, params->wParam, params->lParam);
    sfree(vctx);
}
 ...
 */
static void init_palette(HWND hwnd)
{
    int i;
    HDC hdc = GetDC(hwnd);
    if (hdc) {
 ...
 */
static void general_textout(HWND hwnd, HDC hdc, int x, int y, CONST RECT *lprc,
			    unsigned short *lpString, UINT cbCount,
			    CONST INT *lpDx, bool opaque)
{
    int i, j, xp, xn;
	...
	if (rtl) {
	    exact_textout(hdc, xp, y, lprc, lpString+i, j-i,
                          pData->font_varpitch ? NULL : lpDx+i, opaque);
	} else {
	    ExtTextOutW(hdc, xp, y, ETO_CLIPPED | (opaque ? ETO_OPAQUE : 0),
			lprc, lpString+i, j-i,
			pData->font_varpitch ? NULL : lpDx+i);
	}
...
static int get_font_width(HWND hwnd, HDC hdc, const TEXTMETRIC *tm)
{
    int ret;
	GET_WINDOW_DATA(hwnd);
...
#define LAST '9'
        ...
        pData->font_varpitch = true;
		pData->font_dualwidth = true;
 ...
 */
static void init_fonts(HWND hwnd, int pick_width, int pick_height)
{
    TEXTMETRIC tm;
    CPINFO cpinfo;
    FontSpec *font;
    ...
    pData->bold_font_mode = conf_get_int(conf, CONF_bold_style) & 1 ?
	BOLD_FONT : BOLD_NONE;
    /*bold_colours*/pData->bold_colours = conf_get_int(conf, CONF_bold_style) & 2 ? true : false;
	pData->und_mode = UND_FONT;
    ...
    if (pick_height)
		pData->font_height = pick_height;
    else {
		pData->font_height = font->height;
	if (pData->font_height > 0) {
		pData->font_height =
		-MulDiv(pData->font_height, GetDeviceCaps(hdc, LOGPIXELSY), 72);
	}
    }
    pData->font_width = pick_width;

    quality = conf_get_int(conf, CONF_font_quality);
#define f(i,c,w,u) \
    fonts[i] = CreateFont (pData->font_height, pData->font_width, 0, 0, w, false, u, false, \
    ...
    /* Note that the TMPF_FIXED_PITCH bit is defined upside down :-( */
    if (!(tm.tmPitchAndFamily & TMPF_FIXED_PITCH)) {
		pData->font_varpitch = false;
		pData->font_dualwidth = (tm.tmAveCharWidth != tm.tmMaxCharWidth);
    } else {
		pData->font_varpitch = true;
		pData->font_dualwidth = true;
    }
    if (pick_width == 0 || pick_height == 0) {
		pData->font_height = tm.tmHeight;
		pData->font_width = get_font_width(hwnd, hdc, &tm);
    }
	...
	if (cset == OEM_CHARSET)
		pData->ucsdata.font_codepage = GetOEMCP();
	else
	    if (TranslateCharsetInfo ((DWORD *)(ULONG_PTR)cset,
                                      &info, TCI_SRCCHARSET))
			pData->ucsdata.font_codepage = info.ciACP;
	else
			pData->ucsdata.font_codepage = -1;

	GetCPInfo(pData->ucsdata.font_codepage, &cpinfo);
	pData->ucsdata.dbcs_screenfont = (cpinfo.MaxCharSize > 1);
    }
	...
	und_bm = CreateCompatibleBitmap(hdc, pData->font_width, pData->font_height);
	...
	gotit = false;
	for (i = 0; i < pData->font_height; i++) {
	    c = GetPixel(und_dc, pData->font_width / 2, i);
	    if (c != RGB(0, 0, 0))
		gotit = true;
	}
	...
	DeleteDC(und_dc);
	if (!gotit) {
		pData->und_mode = UND_LINE;
	    DeleteObject(fonts[FONT_UNDERLINE]);
	    fonts[FONT_UNDERLINE] = 0;
	}
    }

    if (pData->bold_font_mode == BOLD_FONT) {
	f(FONT_BOLD, font->charset, fw_bold, false);
    }
#undef f

    pData->descent = tm.tmAscent + 1;
    if (pData->descent >= pData->font_height)
	pData->descent = pData->font_height - 1;

    for (i = 0; i < 3; i++) {
	if (fonts[i]) {
	    if (SelectObject(hdc, fonts[i]) && GetTextMetrics(hdc, &tm))
		fontsize[i] = get_font_width(hwnd, hdc, &tm) + 256 * tm.tmHeight;
	    else
		fontsize[i] = -i;
	} else
	    fontsize[i] = -i;
    ...
    if (pData->trust_icon != INVALID_HANDLE_VALUE) {
	DestroyIcon(pData->trust_icon);
    }
	pData->trust_icon = LoadImage(hinst, MAKEINTRESOURCE(IDI_MAINICON),
			   IMAGE_ICON, pData->font_width *2, pData->font_height,
			   LR_DEFAULTCOLOR);
	...
	fontsize[FONT_BOLD] != fontsize[FONT_NORMAL]) {
		pData->bold_font_mode = BOLD_SHADOW;
    ...
    init_ucs(conf, &(pData->ucsdata));
}

static void another_font(HWND hwnd, int fontno)
{
    int basefont;
    ...
    fonts[fontno] =
	CreateFont(pData->font_height * (1 + !!(fontno & FONT_HIGH)), x, 0, 0, w,
...
static void deinit_fonts(HWND hwnd)
{
	GET_WINDOW_DATA(hwnd);
    int i;
    for (i = 0; i < FONT_MAXNO; i++) {
	...
	pData->trust_icon = INVALID_HANDLE_VALUE;
}

static void wintw_request_resize(TermWin *tw, int w, int h)
{
    int width, height;
	PWindowData pData = get_mapchain_value(window_nodes, tw->hWnd);
    ...
    if (h == pData->term->rows && w == pData->term->cols) return;
	...
	switch (first_time) {
	  case 1:
	    ...
	    if (get_fullscreen_rect(tw->hWnd ,&ss))
		/* first_time = 0 */ ;
	    else {
		first_time = 2;
		break;
	  ...
	  case 0:
	    ...
	    width = (ss.right - ss.left - pData->extra_width) / 4;
	    height = (ss.bottom - ss.top - pData->extra_height) / 6;
    ...
    term_size(pData->term, h, w, conf_get_int(conf, CONF_savelines));
	assert(tw->hWnd);
    if (conf_get_int(conf, CONF_resize_action) != RESIZE_FONT &&
	!IsZoomed(tw->hWnd)) {
	width = pData->extra_width + pData->font_width * w;
	height = pData->extra_height + pData->font_height * h;

	SetWindowPos(tw->hWnd, NULL, 0, 0, width, height,
	    SWP_NOACTIVATE | SWP_NOCOPYBITS |
	    SWP_NOMOVE | SWP_NOZORDER);
    } else
	reset_window(tw->hWnd, 0);

    InvalidateRect(tw->hWnd, NULL, true);
}

static void reset_window(HWND hwnd, int reinit) {
    /*
    ...
    if ( !reinit && 
	    (offset_width != (win_width- pData->font_width * pData->term->cols)/2 ||
	     offset_height != (win_height- pData->font_height* pData->term->rows)/2) ){
	offset_width = (win_width- pData->font_width * pData->term->cols)/2;
	offset_height = (win_height- pData->font_height* pData->term->rows)/2;
	 ...
	 */
		...
		pData->extra_width = wr.right - wr.left - cr.right + cr.left;
	pData->extra_height = wr.bottom - wr.top - cr.bottom + cr.top;

	if (resize_action != RESIZE_TERM) {
	    if (pData->font_width != win_width/ pData->term->cols ||
			pData->font_height != win_height/ pData->term->rows) {
		deinit_fonts(hwnd);
		init_fonts(hwnd, win_width/ pData->term->cols, win_height/ pData->term->rows);
		offset_width = (win_width- pData->font_width * pData->term->cols)/2;
		offset_height = (win_height- pData->font_height* pData->term->rows)/2;
		InvalidateRect(hwnd, NULL, true);
#ifdef RDB_DEBUG_PATCH
		debug("reset_window() -> Z font resize to (%d, %d)\n",
			pData->font_width, pData->font_height);
#endif
	    }
	} else {
	    if (pData->font_width * pData->term->cols != win_width ||
			pData->font_height * pData->term->rows != win_height) {
		/* Our only choice at this point is to change the 
		 * size of the terminal; Oh well.
		 */
		term_size(pData->term, win_height/ pData->font_height, win_width/ pData->font_width,
			  conf_get_int(conf, CONF_savelines));
		offset_width = (win_width- pData->font_width * pData->term->cols)/2;
		offset_height = (win_height- pData->font_height* pData->term->rows)/2;
		InvalidateRect(hwnd, NULL, true);
	...
	if (win_width != pData->font_width * pData->term->cols + offset_width*2 ||
	    win_height != pData->font_height* pData->term->rows + offset_height*2) {
	    ...
	    SetWindowPos(hwnd, NULL, 0, 0, 
			pData->font_width* pData->term->cols + pData->extra_width,
			pData->font_height* pData->term->rows + pData->extra_height,
			 SWP_NOMOVE | SWP_NOZORDER);
	}
	...
	if (win_width != pData->font_width * pData->term->cols + offset_width*2 ||
		...
		get_fullscreen_rect(hwnd, &ss);

	    width = (ss.right - ss.left - pData->extra_width) / pData->font_width;
	    height = (ss.bottom - ss.top - pData->extra_height) / pData->font_height;

	    /* Grrr too big */
	    if (pData->term->rows > height || pData->term->cols > width ) {
		if (resize_action == RESIZE_EITHER) {
		    /* Make the font the biggest we can */
		    if (pData->term->cols > width)
				pData->font_width = (ss.right - ss.left - pData->extra_width)
			    / pData->term->cols;
		    if (pData->term->rows > height)
				pData->font_height = (ss.bottom - ss.top - pData->extra_height)
			    / pData->term->rows;

		    deinit_fonts(hwnd);
		    init_fonts(hwnd, pData->font_width, pData->font_height);

		    width = (ss.right - ss.left - pData->extra_width) / pData->font_width;
		    height = (ss.bottom - ss.top - pData->extra_height) / pData->font_height;
		} else {
		    if ( height > pData->term->rows ) height = pData->term->rows;
		    if ( width > pData->term->cols )  width = pData->term->cols;
		    term_size(pData->term, height, width,
			      conf_get_int(conf, CONF_savelines));
#ifdef RDB_DEBUG_PATCH
	    ...
	    debug("reset_window() -> window resize to (%d,%d)\n",
			pData->font_width*term->cols + extra_width,
			pData->font_height*term->rows + extra_height);
#endif
	}
	return;
    }
    ...
    if (pData->font_width != (win_width-window_border*2)/ pData->term->cols ||
		pData->font_height != (win_height-window_border*2)/ pData->term->rows) {

	deinit_fonts(hwnd);
	init_fonts(hwnd, (win_width-window_border*2)/ pData->term->cols,
		   (win_height-window_border*2)/ pData->term->rows);
	offset_width = (win_width- pData->font_width * pData->term->cols)/2;
	offset_height = (win_height- pData->font_height* pData->term->rows)/2;
                  ...
                  bool shift, bool ctrl, bool alt)
{
	...
	pData->lastbtn = MBT_NOTHING;
	term_mouse(pData->term, b, translate_button(b), MA_CLICK,
		   x, y, shift, ctrl, alt);
	return;
    }

    if (pData->lastbtn == b && thistime - pData->lasttime < pData->dbltime) {
	pData->lastact = (pData->lastact == MA_CLICK ? MA_2CLK :
		   pData->lastact == MA_2CLK ? MA_3CLK :
		   pData->lastact == MA_3CLK ? MA_CLICK : MA_NOTHING);
    } else {
	pData->lastbtn = b;
	pData->lastact = MA_CLICK;
    }
    if (pData->lastact != MA_NOTHING)
	term_mouse(pData->term, b, translate_button(b), pData->lastact,
		   x, y, shift, ctrl, alt);
    pData->lasttime = thistime;
}
...
static void win_seat_notify_remote_exit(Seat *seat)
{
	...
	if (close_on_exit == FORCE_ON ||
	   ...
	   // PostQuitMessage(0);
	    ...
	    if (exitcode != INT_MAX)
		MessageBox(seat->hWnd, "Connection closed by remote host",
				...
				WPARAM wParam, LPARAM lParam)
{
    ...
    int resize_action;
	PWindowData window_data = get_mapchain_value(window_nodes, hwnd);
	...
	return 0;
      case WM_CREATE:
	  {
		  PWindowData windowdata = calloc(1, sizeof(WindowData));
		  windowdata->caret_x = -1;
		  windowdata->caret_y = -1;
		  windowdata->trust_icon = INVALID_HANDLE_VALUE;
		  assert(exist_in_mapchian(window_nodes, hwnd) == false);
		  pmap_node nd = add_to_mapchain(window_nodes, hwnd, windowdata);
		  if (window_nodes == NULL)
			  window_nodes = nd;
		  break;
	  }
      case WM_CLOSE:
	{
		  if (window_nodes) {
			  ...
			  deinit_fonts(hwnd);
			  free(pData);
			  /*if (window_nodes->current->key == hwnd)
				  window_nodes = NULL;*/
			  window_nodes = remove_from_mapchain(window_nodes, hwnd);
			  if (0==get_count_mapchain(window_nodes))
			  {
				  PostQuitMessage(1);
			  }
		  }
	...
	switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
	  ...
	  case IDM_SAVEDSESS:
	    ...
	    break;
	  case IDM_RESTART:
            if (!(window_data->backend)) {
                lp_eventlog(default_logpolicy,
                            "----- Session restarted -----");
		term_pwron(window_data->term, false);
		assert(window_data->term->win);
		start_backend(window_data->term->win);
	    }
	  ...
	  case IDM_RECONF:
		 ...
		 */
		conf_set_str(conf, CONF_wintitle, window_data->window_name);

		prev_conf = conf_copy(conf);

		reconfig_result =
                    do_reconfig(hwnd, window_data->backend ? backend_cfg_info(window_data->backend) : 0, conf);
		reconfiguring = false;
		if (!reconfig_result) {
                    conf_free(prev_conf);
		    break;
		...
		conf_cache_data(&(window_data->cursor_type), &(window_data->vtmode));
		...
		term_reconfig(window_data->term, conf);
                setup_clipboards(window_data->term, conf);

		/* Pass new config data to the back end */
                if (window_data->backend)
                    backend_reconfig(window_data->backend, conf);
		...
		if (conf_get_int(conf, CONF_height) !=
		    ...
		    resize_action == RESIZE_DISABLED)
		    term_size(window_data->term, conf_get_int(conf, CONF_height),
			      conf_get_int(conf, CONF_width),
			      conf_get_int(conf, CONF_savelines));
		...
		/* Enable or disable the scroll bar, etc */
		{
		    ...
		    nflg = flag;
		    if (conf_get_bool(conf, is_full_screen(hwnd) ?
		...
		assert(window_data->term->win);
		win_set_title(window_data->term->win, conf_get_str(conf, CONF_wintitle));
		if (IsIconic(hwnd)) {
		    SetWindowText(hwnd,
				  conf_get_bool(conf, CONF_win_name_always) ?
				  window_data->window_name : window_data->icon_name);
		}
		...
		InvalidateRect(hwnd, NULL, true);
		reset_window(hwnd, init_lvl);

		conf_free(prev_conf);
	    }
	    break;
	  case IDM_COPYALL:
            term_copyall(window_data->term, clips_system, lenof(clips_system));
	    break;
	  case IDM_COPY:
            term_request_copy(window_data->term, clips_system, lenof(clips_system));
	    break;
	  case IDM_PASTE:
	    term_request_paste(window_data->term, CLIP_SYSTEM);
	    break;
	  case IDM_CLRSB:
	    term_clrsb(window_data->term);
	    break;
	  case IDM_RESET:
	  ...
	  default:
		...
		if (i >= n_specials)
		    break;
                if (window_data->backend)
                    backend_special(
						window_data->backend, specials[i].code, specials[i].arg);
	    }
	}
	break;
...
#define TO_CHR_X(x) ((((x)<0 ? (x)-window_data->font_width+1 : (x))-offset_width) / window_data->font_width)
#define TO_CHR_Y(y) ((((y)<0 ? (y)-window_data->font_height+1: (y))-offset_height) / window_data->font_height)
      case WM_LBUTTONDOWN:
      case WM_MBUTTONDOWN:
      case WM_RBUTTONDOWN:
      case WM_LBUTTONUP:
	...
	}
	{
		...
		term_mouse(window_data->term, button, translate_button(button), MA_RELEASE,
	...
	if (wParam & (MK_LBUTTON | MK_MBUTTON | MK_RBUTTON) &&
		...
		b = MBT_RIGHT;
	    term_mouse(window_data->term, b, translate_button(b), MA_DRAG,
		       TO_CHR_X(X_POS(lParam)),
		       TO_CHR_Y(Y_POS(lParam)), wParam & MK_SHIFT,
		       wParam & MK_CONTROL, is_alt_pressed());
	}
	...
	if (!ignore_clip)
	    term_lost_clipboard_ownership(window_data->term, CLIP_SYSTEM);
	ignore_clip = false;
	return 0;
      case WM_PAINT:
	{
	     ...
	     */
            //assert(!wintw_hdc);
            //wintw_hdc = hdc;
		assert(!window_data->term->win->hdc);
		window_data->term->win->hdc = hdc;
	    term_paint(window_data->term,
		       (p.rcPaint.left-offset_width)/window_data->font_width,
		       (p.rcPaint.top-offset_height)/ window_data->font_height,
		       (p.rcPaint.right-offset_width-1)/window_data->font_width,
		       (p.rcPaint.bottom-offset_height-1)/window_data->font_height,
		       !window_data->term->window_update_pending);
           // wintw_hdc = NULL;
		window_data->term->win->hdc = NULL;

	    if (p.fErase ||
	        p.rcPaint.left  < offset_width  ||
		p.rcPaint.top   < offset_height ||
		p.rcPaint.right >= offset_width + window_data->font_width* window_data->term->cols ||
		p.rcPaint.bottom>= offset_height + window_data->font_height* window_data->term->rows)
	    {
		...
		ExcludeClipRect(hdc, 
			...
			offset_width+ window_data->font_width * window_data->term->cols,
			offset_height+window_data->font_height* window_data->term->rows);
	...
	return 0;
      case WM_SETFOCUS:
		  if (window_data == NULL)
			  return 0;
	term_set_focus(window_data->term, true);
	CreateCaret(hwnd, window_data->caretbm, window_data->font_width, window_data->font_height);
	ShowCaret(hwnd);
	flash_window(hwnd, 0);	       /* stop */
	window_data->compose_state = 0;
	term_update(window_data->term);
	break;
      case WM_KILLFOCUS:
	show_mouseptr(true);
	if (window_data)
		term_set_focus(window_data->term, false);
	DestroyCaret();
	if (window_data)
		window_data->caret_x = window_data->caret_y = -1;
	else
		return;
	...
	break;
      case WM_ENTERSIZEMOVE:
	  {
		  ...
		  //enable tip window
		  if (window_data) {
			  if (window_data->tip_wnd) {
				  DestroyWindow(window_data->tip_wnd);
				  window_data->tip_wnd = NULL;
			  }
			  window_data->bEnableTipWnd = true;
		  }
		  resizing = true;
      ...
      case WM_EXITSIZEMOVE:
		  ...
		  if (window_data) {
			  ...
			  window_data->bEnableTipWnd = false;
		  }
	resizing = false;
#ifdef RDB_DEBUG_PATCH
	...
	if (resize_action == RESIZE_TERM ||
		...
		(conf_get_int(conf, CONF_height) != window_data->term->rows ||
		 conf_get_int(conf, CONF_width) != window_data->term->cols)) {
		/* 
		 ...
		 */
	        conf_set_int(conf, CONF_height, window_data->term->rows);
	        conf_set_int(conf, CONF_width, window_data->term->cols);

		InvalidateRect(hwnd, NULL, true);
		need_backend_resize = true;
	    }

	    width = r->right - r->left - window_data->extra_width;
	    height = r->bottom - r->top - window_data->extra_height;
	    w = (width + window_data->font_width / 2) / window_data->font_width;
	    if (w < 1)
		w = 1;
	    h = (height + window_data->font_height / 2) / window_data->font_height;
	    if (h < 1)
		h = 1;
		if (window_data && window_data->bEnableTipWnd) {
			window_data->tip_wnd = UpdateSizeTip(hwnd, window_data->tip_wnd,w, h);
		}
	    ew = width - w * window_data->font_width;
	    eh = height - h * window_data->font_height;
	    if (ew != 0) {
		if (wParam == WMSZ_LEFT ||
	    ...
	    int ex_width = window_data->extra_width + (window_border - offset_width) * 2;
	    int ex_height = window_data->extra_height + (window_border - offset_height) * 2;
	    LPRECT r = (LPRECT) lParam;

	    width = r->right - r->left - ex_width;
	    height = r->bottom - r->top - ex_height;

	    w = (width + window_data->term->cols/2)/ window_data->term->cols;
	    h = (height + window_data->term->rows/2)/ window_data->term->rows;
	    if ( r->right != r->left + w* window_data->term->cols + ex_width)
		rv = 1;

	    if (wParam == WMSZ_LEFT ||
		wParam == WMSZ_BOTTOMLEFT || wParam == WMSZ_TOPLEFT)
		r->left = r->right - w* window_data->term->cols - ex_width;
	    else
		r->right = r->left + w* window_data->term->cols + ex_width;

	    if (r->bottom != r->top + h* window_data->term->rows + ex_height)
		rv = 1;

	    if (wParam == WMSZ_TOP ||
		wParam == WMSZ_TOPRIGHT || wParam == WMSZ_TOPLEFT)
		r->top = r->bottom - h* window_data->term->rows - ex_height;
	    else
		r->bottom = r->top + h* window_data->term->rows + ex_height;
	...
	if (wParam == SIZE_RESTORED || wParam == SIZE_MAXIMIZED)
	    SetWindowText(hwnd, window_data->window_name);
        if (wParam == SIZE_RESTORED) {
            processed_resize = false;
            clear_full_screen(hwnd);
            if (processed_resize) {
                /*
	...
	if (resize_action == RESIZE_DISABLED) {
	    ...
	    height = HIWORD(lParam);
            ...
            if (wParam == SIZE_MAXIMIZED && !(window_data->was_zoomed)) {
				window_data->was_zoomed = true;
                prev_rows = window_data->term->rows;
                prev_cols = window_data->term->cols;
                if (resize_action == RESIZE_TERM) {
                    w = width / window_data->font_width;
                    if (w < 1) w = 1;
                    h = height / window_data->font_height;
                    if (h < 1) h = 1;
                    ...
                    if (resizing) {
                        ...
                        term_size(window_data->term, h, w,
                                  conf_get_int(conf, CONF_savelines));
                    }
                }
                reset_window(hwnd, 0);
            } else if (wParam == SIZE_RESTORED && window_data->was_zoomed) {
				window_data->was_zoomed = false;
                if (resize_action == RESIZE_TERM) {
                    w = (width-window_border*2) / window_data->font_width;
                    if (w < 1) w = 1;
                    h = (height-window_border*2) / window_data->font_height;
                    if (h < 1) h = 1;
                    term_size(window_data->term, h, w, conf_get_int(conf, CONF_savelines));
                    reset_window(hwnd, 2);
	...
	switch (LOWORD(wParam)) {
	  case SB_BOTTOM:
	    term_scroll(window_data->term, -1, 0);
	    break;
	  case SB_TOP:
	    term_scroll(window_data->term, +1, 0);
	    break;
	  case SB_LINEDOWN:
	    term_scroll(window_data->term, 0, +1);
	    break;
	  case SB_LINEUP:
	    term_scroll(window_data->term, 0, -1);
	    break;
	  case SB_PAGEDOWN:
	    term_scroll(window_data->term, 0, +window_data->term->rows / 2);
	    break;
	  case SB_PAGEUP:
	    term_scroll(window_data->term, 0, -window_data->term->rows / 2);
	    break;
	  ...
	  case SB_THUMBTRACK:
		...
		term_scroll(window_data->term, 1, si.nTrackPos);
	    }
	    break;
	}
	break;
	 ...
	 */
	{
		...
		if (len != 0) {
		     ...
		     * we're sent.
		     */
                    term_keyinput(window_data->term, -1, buf, len);
		    show_mouseptr(false);
		}
	    }
	}
		...
		if (window_data->ldisc) {
			...
			if (IS_HIGH_SURROGATE(hs) && i+2 < n) {
			    ...
			    if (IS_LOW_SURROGATE(ls)) {
                                term_keyinputw(
									window_data->term, (unsigned short *)(buff+i), 2);
				i += 2;
				continue;
			    }
			}
                        term_keyinputw(
							window_data->term, (unsigned short *)(buff+i), 1);
                    }
		}
		free(buff);
	    }
	 ...
	 */
	{
	    ...
	    wchar_t c = wParam;
            ...
            if (IS_HIGH_SURROGATE(c)) {
                ...
                pair[1] = c;
                term_keyinputw(window_data->term, pair, 2);
            } else if (!IS_SURROGATE(c)) {
                term_keyinputw(window_data->term, &c, 1);
            }
	}
	return 0;
	...
	if (conf_get_bool(conf, CONF_system_colour)) {
	    ...
	    term_invalidate(window_data->term);
	}
	break;
	...
	process_clipdata(hwnd, (HGLOBAL)lParam, wParam);
	return 0;
      default:
	if (message == wm_mousewheel || message == WM_MOUSEWHEEL) {
		...
		window_data->wheel_accumulator += (short)HIWORD(wParam);
		shift_pressed=LOWORD(wParam) & MK_SHIFT;
		control_pressed=LOWORD(wParam) & MK_CONTROL;
	    } else {
		BYTE keys[256];
		window_data->wheel_accumulator += (int)wParam;
	    ...
	    while (abs(window_data->wheel_accumulator) >= WHEEL_DELTA) {
		int b;

		/* reduce amount for next time */
		if (window_data->wheel_accumulator > 0) {
		    b = MBT_WHEEL_UP;
			window_data->wheel_accumulator -= WHEEL_DELTA;
		} else if (window_data->wheel_accumulator < 0) {
		    b = MBT_WHEEL_DOWN;
			window_data->wheel_accumulator += WHEEL_DELTA;
		} else
		    break;
		...
		if (send_raw_mouse &&
			...
			term_mouse(window_data->term, b, translate_button(b),
				   MA_CLICK,
				   TO_CHR_X(p.x),
		    ...
		    /* trigger a scroll */
		    term_scroll(window_data->term, 0,
				b == MBT_WHEEL_UP ?
				-window_data->term->rows / 2 : window_data->term->rows / 2);
		}
	    }
	    return 0;
	}
...
static void wintw_set_cursor_pos(TermWin *tw, int x, int y)
{
    ...
    if (!pData->term->has_focus) return;

    /*
     * Avoid gratuitously re-updating the cursor position and IMM
     * window if there's no actual change required.
     */
    cx = x * pData->font_width + offset_width;
    cy = y * pData->font_height + offset_height;
    if (cx == pData->caret_x && cy == pData->caret_y)
	return;
    pData->caret_x = cx;
    pData->caret_y = cy;

    sys_cursor_update(tw->hWnd);
}
...
static void sys_cursor_update(HWND hwnd)
{
    ...
    if (pData->caret_x < 0 || pData->caret_y < 0)
	return;

    SetCaretPos(pData->caret_x, pData->caret_y);
    ...
    cf.ptCurrentPos.x = pData->caret_x;
    cf.ptCurrentPos.y = pData->caret_y;
 ...
 */
static void do_text_internal(HWND hWnd, HDC hdc,
    ...
    unsigned long attr, int lattr, truecolour truecolour)
{
    ...
    char_width = fnt_width = pData->font_width * (1 + (lattr != LATTR_NORM));

    if (attr & ATTR_WIDE)
	char_width *= 2;

    /* Only want the left half of double width lines */
    if (lattr != LATTR_NORM && x*2 >= pData->term->cols)
	return;
    ...
    if ((attr & TATTR_ACTCURS) && (pData->cursor_type == 0 || pData->term->big_cursor)) {
    ...
    nfont = 0;
    if (pData->vtmode == VT_POORMAN && lattr != LATTR_NORM) {
	...
	switch ((unsigned char) (text[0])) {
	  case 0xBA:
	    text_adjust = -2 * pData->font_height / 5;
	    break;
	  case 0xBB:
	    text_adjust = -1 * pData->font_height / 5;
	    break;
	  case 0xBC:
	    text_adjust = pData->font_height / 5;
	    break;
	  case 0xBD:
	    text_adjust = 2 * pData->font_height / 5;
	    break;
	...
	if (lattr == LATTR_TOP || lattr == LATTR_BOT)
	    text_adjust *= 2;
	text[0] = pData->ucsdata.unitab_xterm['q'];
	if (attr & ATTR_UNDER) {
	    attr &= ~ATTR_UNDER;
    ...
    if (pData->bold_font_mode == BOLD_FONT && (attr & ATTR_BOLD))
	nfont |= FONT_BOLD;
    if (pData->und_mode == UND_FONT && (attr & ATTR_UNDER))
	nfont |= FONT_UNDERLINE;
    another_font(hWnd, nfont);
    if (!fonts[nfont]) {
    ...
    if (pData->bold_colours && (attr & ATTR_BOLD) && !is_cursor) {
	if (nfg < 16) nfg |= 8;
	else if (nfg >= 256) nfg |= 1;
    }
    if (pData->bold_colours && (attr & ATTR_BLINK)) {
	if (nbg < 16) nbg |= 8;
    ...
    line_box.bottom = y + pData->font_height;
    ...
    /* Only want the left half of double width lines */
    if (line_box.right > pData->font_width * pData->term->cols+offset_width)
	line_box.right = pData->font_width * pData->term->cols+offset_width;

    if (pData->font_varpitch) {
        /*
	...
	if (len > lpDx_len) {
        ...
        if (pData->ucsdata.dbcs_screenfont && (text[0] & CSET_MASK) == CSET_ACP) {
            ...
            for(nlen = mptr = 0; mptr<len; mptr++) {
                uni_buf[nlen] = 0xFFFD;
                if (IsDBCSLeadByteEx(pData->ucsdata.font_codepage,
                                     (BYTE) text[mptr])) {
                    char dbcstext[2];
                    dbcstext[0] = text[mptr] & 0xFF;
                    dbcstext[1] = text[mptr+1] & 0xFF;
                    lpDx[nlen] += char_width;
                    MultiByteToWideChar(pData->ucsdata.font_codepage, MB_USEGLYPHCHARS,
            ...
            ExtTextOutW(hdc, x + xoffset,
                        y - pData->font_height * (lattr == LATTR_BOT) + text_adjust,
                        ETO_CLIPPED | (opaque ? ETO_OPAQUE : 0),
                        &line_box, uni_buf, nlen,
                        lpDx_maybe);
            if (pData->bold_font_mode == BOLD_SHADOW && (attr & ATTR_BOLD)) {
                SetBkMode(hdc, TRANSPARENT);
                ExtTextOutW(hdc, x + xoffset - 1,
                            y - pData->font_height * (lattr ==
            ...
            general_textout(hWnd, hdc, x + xoffset,
                            y - pData->font_height * (lattr==LATTR_BOT) + text_adjust,
    ...
    if (lattr != LATTR_TOP && (force_manual_underline ||
				...
				&& (attr & ATTR_UNDER)))) {
	HPEN oldpen;
	int dec = pData->descent;
	if (lattr == LATTR_BOT)
	    dec = dec * 2 - pData->font_height;
    ...
    unsigned long attr, int lattr, truecolour truecolour)
{
	...
	if (len-len0 >= 1 && IS_LOW_VARSEL(text[len0])) {
	    ...
	    do_text_internal(tw->hWnd, tw->hdc, x, y, text, len0+1, attr, lattr, truecolour);
	    text += len0+1;
	    len -= len0+1;
	    a = TATTR_COMBINING;
	} else if (len-len0 >= 2 && IS_HIGH_VARSEL(text[len0], text[len0+1])) {
	    attr &= ~TATTR_COMBINING;
	    do_text_internal(tw->hWnd, tw->hdc, x, y, text, len0+2, attr, lattr, truecolour);
	    text += len0+2;
	    len -= len0+2;
	    a = TATTR_COMBINING;
	} else {
	...
	while (len--) {
		...
		do_text_internal(tw->hWnd, tw->hdc, x, y, text, 2, attr | a, lattr, truecolour);
		len--;
		text++;
	    } else
                do_text_internal(tw->hWnd, tw->hdc, x, y, text, 1, attr | a, lattr, truecolour);

	    text++;
	    a = TATTR_COMBINING;
	}
    } else
	do_text_internal(tw->hWnd, tw->hdc, x, y, text, len, attr, lattr, truecolour);
}
    ...
    unsigned long attr, int lattr, truecolour truecolour)
{
    ...
    int fnt_width;
    int char_width;
    int ctype = pData->cursor_type;

    lattr &= LATTR_MODE;

    if ((attr & TATTR_ACTCURS) && (ctype == 0 || pData->term->big_cursor)) {
	if (*text != UCSWIDE) {
    ...
    fnt_width = char_width = pData->font_width * (1 + (lattr != LATTR_NORM));
    if (attr & ATTR_WIDE)
	char_width *= 2;
    x *= fnt_width;
    y *= pData->font_height;
    ...
    if ((attr & TATTR_PASCURS) && (ctype == 0 || pData->term->big_cursor)) {
	POINT pts[5];
	HPEN oldpen;
	pts[0].x = pts[1].x = pts[4].x = x;
	pts[2].x = pts[3].x = x + char_width - 1;
	pts[0].y = pts[3].y = pts[4].y = y;
	pts[1].y = pts[2].y = y + pData->font_height - 1;
	oldpen = SelectObject(tw->hdc, CreatePen(PS_SOLID, 0, colours[261]));
	Polyline(tw->hdc, pts, 5);
	oldpen = SelectObject(tw->hdc, oldpen);
	DeleteObject(oldpen);
    } else if ((attr & (TATTR_ACTCURS | TATTR_PASCURS)) && ctype != 0) {
	int startx, starty, dx, dy, length, i;
	if (ctype == 1) {
	    startx = x;
	    starty = y + pData->descent;
	    dx = 1;
	    dy = 0;
	    length = char_width;
	} else {
	    ...
	    dx = 0;
	    dy = 1;
	    length = pData->font_height;
	}
	if (attr & TATTR_ACTCURS) {
	    HPEN oldpen;
	    oldpen =
		SelectObject(tw->hdc, CreatePen(PS_SOLID, 0, colours[261]));
	    MoveToEx(tw->hdc, startx, starty, NULL);
	    LineTo(tw->hdc, startx + dx * length, starty + dy * length);
	    oldpen = SelectObject(tw->hdc, oldpen);
	    DeleteObject(oldpen);
	} else {
	    for (i = 0; i < length; i++) {
		if (i % 2 == 0) {
		    SetPixel(tw->hdc, startx, starty, colours[261]);
		}
		startx += dx;
		starty += dy;
	    }
...
static void wintw_draw_trust_sigil(TermWin *tw, int x, int y)
{
    ...
    DrawIconEx(tw->hdc, x, y, pData->trust_icon, pData->font_width * 2, pData->font_height,
               0, NULL, DI_NORMAL);
}
...
static int wintw_char_width(TermWin *tw, int uc)
{
    ...
    if (!(pData->font_dualwidth)) return 1;

    switch (uc & CSET_MASK) {
      case CSET_ASCII:
	uc = pData->ucsdata.unitab_line[uc & 0xFF];
	break;
      case CSET_LINEDRW:
	uc = pData->ucsdata.unitab_xterm[uc & 0xFF];
	break;
      case CSET_SCOACS:
	uc = pData->ucsdata.unitab_scoacs[uc & 0xFF];
	break;
    }
    if (DIRECT_FONT(uc)) {
	if (pData->ucsdata.dbcs_screenfont) return 1;
	...
	if ( (uc & CSET_MASK) == CSET_ACP ) {
	    SelectObject(tw->hdc, fonts[FONT_NORMAL]);
	} else if ( (uc & CSET_MASK) == CSET_OEMCP ) {
	    another_font(tw->hWnd, FONT_OEM);
	    if (!fonts[FONT_OEM]) return 0;

	    SelectObject(tw->hdc, fonts[FONT_OEM]);
	} else
	    return 0;

	if (GetCharWidth32(tw->hdc, uc & ~CSET_MASK,
                           uc & ~CSET_MASK, &ibuf) != 1 &&
	    GetCharWidth(tw->hdc, uc & ~CSET_MASK,
                         uc & ~CSET_MASK, &ibuf) != 1)
	    return 0;
    } else {
	/* Speedup, I know of no font where ascii is the wrong width */
	...
	SelectObject(tw->hdc, fonts[FONT_NORMAL]);
	if (GetCharWidth32W(tw->hdc, uc, uc, &ibuf) == 1)
	    /* Okay that one worked */ ;
	else if (GetCharWidthW(tw->hdc, uc, uc, &ibuf) == 1)
	    /* This should work on 9x too, but it's "less accurate" */ ;
	else
	    return 0;
    }

    ibuf += pData->font_width / 2 -1;
    ibuf /= pData->font_width;
 ...
 */
static int TranslateKey(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam,
			unsigned char *output)
{
    BYTE keystate[256];
...
#define SHOW_TOASCII_RESULT
	...
	if ((funky_type == FUNKY_VT400 ||
	     (funky_type <= FUNKY_LINUX && pData->term->app_keypad_keys &&
	      !no_applic_k))
	...
	if (pData->compose_state == 0
	    && (HIWORD(lParam) & (KF_UP | KF_REPEAT)) == 0) pData->compose_state =
		1;
	else if (pData->compose_state == 1 && (HIWORD(lParam) & KF_UP))
		pData->compose_state = 2;
	else
		pData->compose_state = 0;
    } else if (pData->compose_state == 1 && wParam != VK_CONTROL)
		pData->compose_state = 0;

    if (pData->compose_state > 1 && left_alt)
		pData->compose_state = 0;

    /* Sanitize the number pad if not using a PC NumPad */
    if (left_alt || (pData->term->app_keypad_keys && !no_applic_k
		     && funky_type != FUNKY_XTERM)
	|| funky_type == FUNKY_VT400 || nethack_keypad || pData->compose_state) {
    ...
    if (key_down && (keystate[VK_RMENU] & 0x80) == 0 && !(pData->compose_state)) {
	...
	if ((wParam == VK_PRIOR || wParam == VK_NEXT) && shift_state == 3) {
	    term_scroll_to_selection(pData->term, (wParam == VK_PRIOR ? 0 : 1));
	    return 0;
	}
	if (wParam == VK_INSERT && shift_state == 2) {
            switch (conf_get_int(conf, CONF_ctrlshiftins)) {
              ...
              case CLIPUI_EXPLICIT:
                term_request_copy(pData->term, clips_system, lenof(clips_system));
                break;
              default:
                break;
            }
	...
	if (wParam == VK_INSERT && shift_state == 1) {
            switch (conf_get_int(conf, CONF_ctrlshiftins)) {
              case CLIPUI_IMPLICIT:
                term_request_paste(pData->term, CLIP_LOCAL);
                break;
              case CLIPUI_EXPLICIT:
                term_request_paste(pData->term, CLIP_SYSTEM);
                break;
              default:
                break;
            }
	...
	if (wParam == VK_PAUSE && shift_state == 2) {
		pData->term->app_keypad_keys = !pData->term->app_keypad_keys;
	    return 0;
	}
	...
	if (wParam == VK_CANCEL && shift_state == 2) {	/* Ctrl-Break */
            if (pData->backend)
                backend_special(pData->backend, SS_BRK, 0);
	    return 0;
	}
	...
	switch (wParam) {
	  ...
	  case VK_F20: fkey_number = 20; goto numbered_function_key;
          numbered_function_key:
            p += format_function_key((char *)p, pData->term, fkey_number,
                                     shift_state & 1, shift_state & 2);
            return p - output;
	  ...
	  case VK_NEXT: sk_key = SKK_PGDN; goto small_keypad_key;
          small_keypad_key:
            ...
            p += format_small_keypad_key((char *)p, pData->term, sk_key);
            return p - output;
	    ...
	    char xkey;
          ...
          arrow_key:
            p += format_arrow_key((char *)p, pData->term, xkey, shift_state & 2);
            return p - output;
          ...
          ordinary_return_key:
            if (shift_state == 0 && pData->term->cr_lf_return) {
                *p++ = '\r';
                *p++ = '\n';
                return p - output;
            } else {
	...
	if (r > 0) {
		...
		if (pData->compose_state == 2 && wch >= ' ' && wch < 0x80) {
		    compose_char = wch;
			pData->compose_state++;
		    continue;
		}
		if (pData->compose_state == 3 && wch >= ' ' && wch < 0x80) {
		    int nc;
			pData->compose_state = 0;
		    ...
		    keybuf = nc;
                    term_keyinputw(pData->term, &keybuf, 1);
		    continue;
		}
		...
		if (!key_down) {
		    if (alt_sum) {
			if (in_utf(pData->term) || pData->ucsdata.dbcs_screenfont) {
			    keybuf = alt_sum;
                            term_keyinputw(pData->term, &keybuf, 1);
			} else {
			     ...
			     */
			    term_keyinput(pData->term, -1, &ch, 1);
			}
			alt_sum = 0;
		    } else {
			term_keyinputw(pData->term, &wch, 1);
		    }
		} else {
		    if(capsOn && wch < 0x80) {
			WCHAR cbuf[2];
			...
			term_keyinputw(pData->term, cbuf+!left_alt, 1+!!left_alt);
		    } else {
			WCHAR cbuf[2];
			cbuf[0] = '\033';
			cbuf[1] = wch;
	...
	else if (keys_unicode[0] && (in_utf(pData->term) || pData->ucsdata.dbcs_screenfont))
	    keys_unicode[0] = 10;
    }
...
{
	GET_WINDOW_DATA(tw->hWnd);
    sfree(pData->window_name);
	pData->window_name = snewn(1 + strlen(title), char);
    strcpy(pData->window_name, title);
    if (conf_get_bool(conf, CONF_win_name_always) || !IsIconic(tw->hWnd))
	SetWindowText(tw->hWnd, title);
}

static void wintw_set_icon_title(TermWin *tw, const char *title)
{
	GET_WINDOW_DATA(tw->hWnd);
    sfree(pData->icon_name);
	pData->icon_name = snewn(1 + strlen(title), char);
    strcpy(pData->icon_name, title);
    if (!conf_get_bool(conf, CONF_win_name_always) && IsIconic(tw->hWnd))
	SetWindowText(tw->hWnd, title);
}
...
static void wintw_set_scrollbar(TermWin *tw, int total, int start, int page)
{
    ...
    if (!conf_get_bool(conf, is_full_screen(tw->hWnd) ?
...
{
    //assert(!make_hdc(tw->hWnd););
	//assert(!tw->hdc);
	tw->hdc = make_hdc(tw->hWnd);
    //wintw_hdc = make_hdc(tw->hWnd);
    return tw->hdc != NULL;
}
...
static void wintw_free_draw_ctx(TermWin *tw)
{
    //assert(wintw_hdc);
	assert(tw->hdc);
    //free_hdc(tw->hWnd, wintw_hdc);
	free_hdc(tw->hWnd, tw->hdc);
    //wintw_hdc = NULL;
	tw->hdc = NULL;
}
...
static void wintw_palette_set(TermWin *tw, int n, int r, int g, int b)
{
    ...
    if (pal) {
	HDC hdc = make_hdc(tw->hWnd);
	UnrealizeObject(pal);
	RealizePalette(hdc);
	free_hdc(tw->hWnd,hdc);
    } else {
...
void write_aclip(HWND hwnd, int clipboard, char *data, int len, bool must_deselect)
{
    HGLOBAL clipdata;
    void *lock;
    ...
    truecolour *truecolour, int len, bool must_deselect)
{
	...
	if (attr) {
		...
		if (pData->bold_colours && (attr[i] & ATTR_BOLD)) {
	...
	while (tindex < len2 && uindex < len &&
		...
		if (pData->bold_colours && (attr[tindex] & ATTR_BOLD) && (fgcolour >= 0)) {
		    if (fgcolour  <   8)	    /* ANSI colours */
			fgcolour +=   8;
		    else if (fgcolour >= 256)	    /* Default colours */
			fgcolour ++;
		...
		if ((attr[tindex] & ATTR_BLINK) && (bgcolour >= 0)) {
			...
			bgcolour ++;
                 ...
                 */
		if (pData->bold_font_mode != BOLD_NONE)
		...
		if (!(attr[tindex] & ATTR_REVERSE)) {
			...
			if (pData->bold_colours && (fgcolour & 1) && bgcolour == -1)
    ...
    if (!must_deselect)
	SendMessage(tw->hWnd, WM_IGNORE_CLIP, true, 0);

    if (OpenClipboard(tw->hWnd)) {
	EmptyClipboard();
    ...
    if (!must_deselect)
	SendMessage(tw->hWnd, WM_IGNORE_CLIP, false, 0);
}
...
static void process_clipdata(HWND hwnd, HGLOBAL clipdata, bool unicode)
{
	...
	if (p) {
	    ...
	    term_do_paste(pData->term, clipboard_contents, clipboard_length);
	}
    } else {
...
void modalfatalbox(const char *fmt, ...)
{
    ...
    MessageBox(NULL, message, title, MB_SYSTEMMODAL | MB_ICONERROR | MB_OK);
    sfree(message);
    sfree(title);
    cleanup_exit(NULL, 1);
}
...
static bool flash_window_ex(HWND hwnd, DWORD dwFlags, UINT uCount, DWORD dwTimeout)
{
    if (p_FlashWindowEx) {
	FLASHWINFO fi;
	fi.cbSize = sizeof(fi);
...
static void flash_window(HWND, int mode);
...
{
	PWindowData pData = get_mapchain_value(window_nodes, ctx);
    if (pData->flashing && now == next_flash) {
	flash_window((HWND)ctx, 1);
    }
}
...
static void flash_window(HWND hwnd, int mode)
{
	...
	if (pData->flashing) {
		pData->flashing = false;
	    if (p_FlashWindowEx)
		flash_window_ex(hwnd, FLASHW_STOP, 0, 0);
	    else
	...
	if (!(pData->flashing)) {
		 ...
		 * documented. */
		flash_window_ex(hwnd, FLASHW_ALL | FLASHW_TIMER,
				(beep_ind == B_IND_FLASH ? 0 : 2),
				0 /* system cursor blink rate */);
		/* No need to schedule timer */
	    } else {
		FlashWindow(hwnd, true);
		next_flash = schedule_timer(hwnd, 450, flash_window_timer, hwnd);
	    }
	}

    } else if ((mode == 1) && (beep_ind == B_IND_FLASH)) {
	/* maintain */
	if (pData->flashing && !p_FlashWindowEx) {
...
static void wintw_bell(TermWin *tw, int mode)
{
	...
	if (!p_PlaySound || !p_PlaySound(bell_wavefile->path, NULL,
            ...
            MessageBox(tw->hWnd, buf, otherbuf, MB_OK | MB_ICONEXCLAMATION);
            sfree(buf);
            sfree(otherbuf);
    ...
    if (!pData->term->has_focus) {
	flash_window(tw->hWnd, 2);	       /* start */
    }
}
...
static void wintw_set_minimised(TermWin *tw, bool minimised)
{
	...
	if (minimised)
	    ShowWindow(tw->hWnd, SW_MINIMIZE);
    }
}
...
static void wintw_move(TermWin *tw, int x, int y)
{
    ...
    SetWindowPos(tw->hWnd, NULL, x, y, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
}
...
static void wintw_set_zorder(TermWin *tw, bool top)
{
    if (conf_get_bool(conf, CONF_alwaysontop))
	return;			       /* ignore */
    SetWindowPos(tw->hWnd, top ? HWND_TOP : HWND_BOTTOM, 0, 0, 0, 0,
		 SWP_NOMOVE | SWP_NOSIZE);
}
...
static void wintw_set_maximised(TermWin *tw, bool maximised)
{
	...
	if (maximised)
	    ShowWindow(tw->hWnd, SW_MAXIMIZE);
    }
}
...
{
	GET_WINDOW_DATA(tw->hWnd);
    return icon ? pData->icon_name : pData->window_name;
}

/*
 * See if we're in full-screen mode.
 */
static bool is_full_screen(HWND hwnd)
{
    if (!IsZoomed(hwnd))
	return false;
...
static bool get_fullscreen_rect(HWND hwnd, RECT * ss)
{
 ...
 */
static void clear_full_screen(HWND hwnd)
{
 ...
 */
static void flip_full_screen(HWND hwnd)
{
    if (is_full_screen(hwnd)) {
	ShowWindow(hwnd, SW_RESTORE);
...
{
	GET_WINDOW_DATA(seat->hWnd);
    return term_data(pData->term, is_stderr, data, len);
}
    ...
    Seat *seat, prompts_t *p, bufchain *input)
{
    ...
    if (ret == -1)
	ret = term_get_userpass_input(pData->term, p, input);
    return ret;
}
...
{
	GET_WINDOW_DATA(seat->hWnd);
    term_set_trust_status(pData->term, trusted);
    return true;
}
