...
static void random_timer(void *ctx, unsigned long now)
{
	...
	next_noise_collection =
	    schedule_timer(hwnd, NOISE_REGULAR_INTERVAL, random_timer,
                           (void*)hwnd);
    }
}
...
static void random_create(HWND hwnd, const ssh_hashalg *hashalg)
{
    assert(!global_prng);
...
void random_ref(HWND hwnd)
{
    if (!random_active++)
        random_create(hwnd, &ssh_sha256);
}

void random_setup_special()
{
    random_active++;
    random_create(NULL, &ssh_sha512);
}
