...
static void term_schedule_update(Terminal *term)
{
	...
	term->next_update = schedule_timer(term->win->hWnd, UPDATE_DELAY, term_timer, term);
    }
}
...
static void term_schedule_tblink(Terminal *term)
{
	...
	if (!term->tblink_pending)
	    term->next_tblink = schedule_timer(term->win->hWnd, TBLINK_DELAY, term_timer, term);
...
static void term_schedule_cblink(Terminal *term)
{
	...
	if (!term->cblink_pending)
	    term->next_cblink = schedule_timer(term->win->hWnd, CBLINK_DELAY, term_timer, term);
				...
				long startpoint)
{
	...
	term->in_vbell = true;
	term->vbell_end = schedule_timer(term->win->hWnd, VBELL_DELAY - ticks_already_gone,
					 term_timer, term);
    } else {
	term->in_vbell = false;
    }
